...
7.   Residuals
    ...
    The residuals and standardized values (as well as the predicted values, the confidence

    interval endpoints, and the prediction interval endpoints) can be found in the data window.
    ...
    ![](images/Residual_plot.PNG){width="100%"}

    ![](images/studentized_regression_plot.PNG)

    Also, to assess the normality of the residuals, consult the P-P Plot from the regression output.

    ![](images/normal_p-p_plot.PNG)

    **Interpretation**- The residual plot shows a random scatter of the points (independence) with a constant spread (variance). Also, the studentized residual plot shows a random scatter of the points (independence) with a constant variance and with no values beyond the ±2 standard

    deviation reference lines (no outliers). The normal probability plot of the residuals shows the points

    close to a diagonal line; therefore, the residuals appear to be approximately normally distributed.

    Thus, we have evidence to conclude that the assumptions for regression analysis appear to be met.
